...
function HeaderTemplate(props: iProps) {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const response = await fetch(
        `http://0.0.0.0:8080/users/${userId}`,
        customSettings
      );
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:justify-between gap-4 border-b border-yellow-500 mb-4">
      <LogoAtom
        className="self-center w-72 sm:w-96"
        image="public/assets/Noodler-logo.png"></LogoAtom>
      <div className="flex flex-col self-center sm:w-2/3">
        <DisplayAtom
          className="text-3xl sm:text-6xl"
          text={username}></DisplayAtom>
        <DisplayAtom
          className="text-base sm:text-2xl"
          text={description}></DisplayAtom>
      </div>
    </div>
  );
}
